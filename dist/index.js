...
function allJobReferences(value) {
  ...
  return [...text.matchAll(/\bneeds\.([A-Za-z0-9_-]+)\b/g)].map((match) => match[1]).filter((value2) => value2 !== void 0);
}
function secretNames(value) {
  const text = expressionText(value);
  return [...new Set([...text.matchAll(/\bsecrets\.([A-Za-z0-9_]+)\b/g)].map((match) => match[1]).filter((name) => name !== void 0))].sort();
}
...
function sharedCoreDetections(workflow, repoPath) {
  let source = "";
  try {
    const absolute = repoPath ? join3(repoPath, workflow.path) : workflow.path;
...
async function changeLocalDetections(ctx, detections) {
  ...
  const changedFiles = new Set(ctx.change.changedFiles.map(normalizePath3));
  const directPaths = [...new Set(detections.filter((detection) => detection.locality?.kind === "direct").map((detection) => normalizePath3(detection.file)).filter((path) => changedFiles.has(path)))];
...
function isEffectiveCancellation(value) {
  return isRecord2(value) && value["cancel-in-progress"] === true && value.group !== void 0;
}
...
async function inspectRepository(repoPath, files) {
  ...
  const dockerfiles = await Promise.all(
    dockerfilePaths.map(async (path) => ({ path, source: await readFile4(join4(repoPath, path), "utf8") }))
  );
  return {
    files: new Set(files),
...
async function discoverDepotWorkflows(repoPath) {
  ...
  const failures = [];
  for (const path of candidates) {
    const source = await readFile5(join5(repoPath, path), "utf8");
...
async function walkRepository(root, limit = MAX_DISCOVERY_FILES) {
  ...
  async function visit(relativeDirectory) {
    ...
    const directory = join5(root, relativeDirectory);
    ...
    for (const entry of entries) {
      ...
      const relativePath = relativeDirectory.length === 0 ? entry.name : join5(relativeDirectory, entry.name);
      if (entry.isDirectory()) {
